function ssm_connect
        if test -n "(crudini --get ~/.aws/credentials | grep arn:aws:sts)"
          set -g AWS_PROFILE (crudini --get ~/.aws/credentials | grep arn:aws:sts)
        ...
        if test -z "$AWS_PROFILE"
          set -g AWS_PROFILE (cat ~/.aws/config | grep -Po "(?<=\\[).*(?=\\])" | grep "profile" | cut -f2 -d' ' | fzf -e --height 1% --header 'Select AWS profile' --reverse --info=hidden)
